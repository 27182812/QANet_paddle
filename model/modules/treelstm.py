...
class BinaryTreeLSTMLayer(nn.Module):
    ...
    #     self_weight = nn.initializer.KaimingNormal()
    #     self_bais = nn.initializer.Constant(0)
    ...
    def forward(self, left=None, right=None):
        ...
        hl, cl = left
        hr, cr = right
        hlr_cat = paddle.concat([hl, hr], dim=2)
    ...
    def __init__(self, word_dim, hidden_dim, use_leaf_rnn, intra_attention,
        ...
        if use_leaf_rnn:
            self.leaf_rnn_cell = nn.LSTMCell(
                ...
                weight_ih_attr=nn.initializer.KaimingNormal(),
                bias_hh_attr=nn.initializer.Constant(0),
                bias_ih_attr=nn.initializer.Constant(0)
            )
            if bidirectional:
        ...
        if self.bidirectional:
            ...
            self.comp_query = self.create_parameter(
                shape=[2 * hidden_dim], dtype="float32"
            )
