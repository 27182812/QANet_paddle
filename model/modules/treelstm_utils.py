...
def affine_nd(input, weight, bias):
    ...
    output_flat = paddle.addmm(bias_expand, input_flat, weight)
...
def dot_nd(query, candidates):
    ...
    output_flat = mv(cands_flat, query)
...
def st_gumbel_softmax(logits, temperature=1.0, mask=None):
    ...
    gumbel_noise = -paddle.log(-paddle.log(u + eps) + eps)
...
def sequence_mask(seq_length, max_length=None):
    ...
    seq_range = paddle.arange(0, max_length).long()
