...
def collate(data):
    ...
    Cwid = paddle.to_tensor(Cwid).astype("int64")
    Ccid = paddle.to_tensor(Ccid).astype("int64")
    Qwid = paddle.to_tensor(Qwid).astype("int64")
    Qcid = paddle.to_tensor(Qcid).astype("int64")
    y1 = paddle.to_tensor(np.array(y1)).astype("int64")
    y2 = paddle.to_tensor(np.array(y2)).astype("int64")
    id = paddle.to_tensor(np.array(id)).astype("int64")
    answerable = paddle.to_tensor(answerable).astype("int64")
